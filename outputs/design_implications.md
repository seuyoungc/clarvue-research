...
This document distills the patterns surfaced in [notebooks/analysis.ipynb](../notebooks/analysis.ipynb) into design directions for any planning tool aimed at the luteal-phase experience. It is not a clinical claim — it is a synthesis of what people in r/PMDD, r/pms, and r/Periods describe when they talk about planning, deciding, and coping during the luteal phase.

**Method, in three lines.** ~500 public posts collected April 2026 from three subreddits across seven planning- and cognition-related search terms. Mixed computational coding (TF-IDF + KMeans, VADER) and manual flagging for `pain_point`, `coping`, `unmet_need`, and `app_signal`. Anonymized: usernames never collected, URLs and timestamps stripped from the published dataset, quotes paraphrased.

User-testing rubrics derived from these findings live in [test_protocol.md](test_protocol.md).
...
500+ posts, three subreddits (`r/PMDD`, `r/pms`, `r/Periods`), seven planning- and cognition-related search terms. Mixed computational (TF-IDF + KMeans for thematic structure, VADER for valence) and manual coding for `pain_point`, `coping`, `unmet_need`, and `app_signal` flags. Cluster-level summarization, sentiment by phase-related language, and qualitative deep-reads of the highest-engagement and most-distressed posts. Anonymization: usernames never collected, post URLs and timestamps stripped from the published dataset, displayed quotes paraphrased.
